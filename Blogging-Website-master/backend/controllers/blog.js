...
exports.create = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (!title || title.length < 3) {
      return res.status(400).json({
        error: "Title should be of atleast 3 characters",
      });
    }

    if (!body || body.length < 50) {
      return res.status(400).json({
        error: "Content is too short. Should be of atleast 50 words",
      });
    }
    ...
    blog.save((err, result) => {
      if (err) {
        return res.status(400).json({
          error: "Blog already exists. Try creating different one",
